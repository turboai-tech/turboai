...
| 登录限流 | **只能在 Supabase Dashboard 配置** —— 见下方说明 | P1 |
| 自有接口限流 | 留资提交、微信回调按 IP 限流（已实现） | P1 |
| 忘记 / 重置密码 | `resetPasswordForEmail` + 独立页面 | P1 |
| 审计日志 | `auth_events` 表，只记服务端可观察的事件（已实现） | P2 |
...
实现过程中确认了一件事，它推翻了本节的初稿：

**登录与密码重置是浏览器直连 Supabase 的。** `NEXT_PUBLIC_SUPABASE_URL` 和
anon key 按设计就会打进前端包，任何人都能从 JS 里读出来直接调用 Supabase 的
auth 接口。在我们的应用里加限流，只拦得住老老实实走 UI 的人 —— 攻击者绕过
我们即可，属于安全剧场。
...
| 目标 | 由谁负责 |
|---|---|
| 登录 / 注册 / 密码重置的频率限制 | **Supabase Dashboard → Authentication → Rate Limits** |
| 密码强度策略 | **Supabase Dashboard → Authentication → Policies** |
| 留资提交、微信回调 | 应用层（`server/security.ts`，已实现） |

应用层限流只保护我们**自己拥有、且没有上游保护**的公开写接口。

**另一条不变的要求**：登录失败必须是「慢失败」，且错误信息不区分「用户不存在」和「密码错误」—— 否则接口就成了账号枚举器。这条在实现时容易被「体验优化」的诉求推翻，需要写进 review 清单。
...
**M3 — 密码找回与账号安全**（已完成）
- ~~忘记密码 / 重置密码两个页面~~
- ~~审计日志表与写入~~ —— 只记服务端可观察的事件。登录/登出发生在浏览器与
  Supabase 之间，我们看不到；那部分查 Supabase 自带的 Auth Logs，在这里补一份
  客户端自报的记录只会得到一份可被伪造的日志，比没有更糟。
- ~~限流~~ —— 范围调整为自有公开接口，登录限流交由 Supabase Dashboard，
  理由见第 5 节。
